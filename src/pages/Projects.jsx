...
function renderProjectActions(title, liveUrl, sourceUrl, compact = false) {
  return (
    <div className={`flex flex-col sm:flex-row flex-wrap gap-3 ${compact ? "mt-auto pt-4" : "mt-6"}`}>
      <a href={liveUrl} target="_blank" rel="noopener noreferrer" className={`${actionClass} bg-primary text-green-100 hover:bg-dark focus-visible:outline-primary`}>
        Live demo<span className="sr-only"> for {title}</span>
      </a>
      <a href={sourceUrl} target="_blank" rel="noopener noreferrer" className={`${actionClass} border border-primary text-primary hover:bg-primary/10 focus-visible:outline-primary`}>
        Source code<span className="sr-only"> for {title}</span>
      </a>
    </div>
  );
}
  ...
  return (
    <section id="projects" aria-labelledby="projects-heading" className="relative bg-backdrop py-12 sm:py-16 md:py-20">
      <div className="container mx-auto px-4 sm:px-6">
        <div className="max-w-3xl text-center mx-auto text-primary">
        ...
        <article className="mt-10 md:mt-14 overflow-hidden rounded-2xl border border-primary/15 bg-white/60 shadow-lg shadow-primary/10">
          <div className="grid lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
            <div className="relative min-h-64 lg:min-h-full bg-primary/10">
            ...
            <div className="p-5 sm:p-8 lg:p-10 text-primary">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-px bg-primary/15 border-t border-primary/15">
        ...
        <section aria-labelledby="selected-projects-heading" className="mt-14 md:mt-20">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 md:gap-8 mt-8">
            {projectData.map((project) => (
              <article key={project.id} className="rounded-xl overflow-hidden border border-primary/15 bg-white/50 flex flex-col">
                <img src={project.image} width={project.imageWidth} height={project.imageHeight} alt={project.imageAlt} loading="lazy" decoding="async" className="w-full aspect-video object-cover" />
