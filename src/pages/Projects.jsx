...
export default function Projects() {
  return (
    ...
    <section className='relative bg-backdrop flex items-center py-10 pt-52 md:pt-6'>
    ...
    <p className='text-xl md:text-2xl text-center text-balance leading-relaxed'> Welcome to my Projects section! Here, you&apos;ll find a curated selection of my recent work, showcasing a diverse range of web development projects. Each project highlights my skills in front-end development, user experience design, and problem-solving. Click on any project title to explore the live version and see the code in action.</p>
    <section className="container mx-auto px-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 py-10">
        {
            projectData.map((el,i) =>( <aside data-aos={i % 2 === 0 ? "zoom-in-left" : "zoom-out-right"} data-aos-delay={(i+1)*900} data-aos-duration="1000" key={el.id} className="bg-backdrop p-4 md:p-6 rounded-md flex flex-col gap-4 md:gap-6 items-center text-center hover:-translate-y-4">
              <img src={el.image} alt='' className=" border border-2px left-o top-0 mb-4 rounded-md w-full h-full object-cover" />  
              <a
                  href={el.link}
                  target="_blank"
